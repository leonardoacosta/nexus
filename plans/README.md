...
| 035 | Convert beads-watcher.ts + bead-rollup.ts to async fs and batch the startup recount fan-out | P2 | S | — | DONE (main, unpushed; drift check clean against 089e0338; `collectLinkedBeadIds`/`computeBeadCountsFromDisk`/`recount` converted to async fs (readdir/readFile/stat), 3 `recount` call sites (poll timer, debounce, startup IIFE) fire-and-forget `.catch()`-wrapped per this file's existing idiom, `startBeadsWatcher`'s sync-returning signature untouched; startup fan-out batched via local `BATCH_SIZE=4`/`BATCH_DELAY_MS=200` consts (NOT imported from spec-watcher/constants.ts, matching its own "intentionally local" docstring); scoped suite (beads-watcher.test.ts + bead-rollup.test.ts + beads-unlinked.test.ts) 39/0; `pnpm --filter @nexus/agent typecheck` carries only the 2 pre-existing unrelated WatcherPool.updateSecret errors plans 032/033/039 already document, confirmed via git-stash isolation; plan's own Step-1 import snippet had a defect (would have dropped readFileSync/readdirSync out from under the untouched, out-of-scope resolveTasksMd) — corrected to keep both sync and promise imports side by side, zero behavior change to resolveTasksMd; grep-convention: 0 sync-fs in beads-watcher.ts, sync-fs in bead-rollup.ts confined to resolveTasksMd, BATCH_SIZE/BATCH_DELAY_MS local, both collectLinkedBeadIds call sites await'd; repo-wide `bun test` contended at verification time — 3 unrelated concurrent bun test processes already running 14-43min at ~5% CPU on this shared machine (DB-lock contention across concurrent sessions, not attributable to this diff); not pushed per plan instruction) |
| 036 | Document 6 operator-facing env vars missing from .env.example (retention-days + poll-intervals) | P3 | S | — | DONE (converted into openspec proposal `close-env-doc-and-audit-suppression-drift` and applied via `/apply:all` apply-2026-07-13-003 wave 1; main @ d41a9943. `.env.example` H1 audit-scan count 17→11, all 6 owned vars documented, `NEXUS_PHONE_PEER`/`NEXUS_PRESENCE_USER` correctly left off per plan 022's still-standing deferral) |
| 037 | Add narrow A2 suppression entry for 4 guarded test-skip-reason console.log sites | P3 | S | — | DONE (converted into openspec proposal `close-env-doc-and-audit-suppression-drift` and applied via `/apply:all` apply-2026-07-13-003 wave 1; main @ d41a9943. `.audit-suppressions.json` +1 path-scoped A2 stanza, `A2` NOT added to `autoSkipTestFiles`; `packages/core/src/audit-suppressions.integration.test.ts` "A2 finding count is zero" flipped FAIL→pass) |
...
| 041 | Downgrade repeat TTS notifications to silent desktop when a project exceeds a rate threshold | P2 | L | — | DONE (main, unpushed; drift check clean against 6796f8ab — only unrelated drift in buffer.ts (deliveryState, plan 038) and notifications.ts (DEDUP_TTL_MS, plan 039), neither in the touched regions; 3-column migration 0054_nice_trauma.sql generated (exactly 3 ADD COLUMN on notification_settings, no hand-edits); notification-settings.test.ts 34/0 (31 existing + 3 new); buffer.test.ts 6/0 (4 existing + 2 new countRecentNotifications); manager-rate-throttle.test.ts 6/0 new (disabled/under-threshold/at-threshold/critical-exempt/project-less-exempt/no-wiring); full `bun test apps/agent/src/notifications/` 218/0/17skip; crossMachine was NOT wired at the initNotificationRoutes call site (confirmed by reading the live file) — passed explicitly as undefined per the plan's STOP-condition guard, no regression; `pnpm --filter @nexus/agent typecheck` exits 2 but independently confirmed pre-existing/unrelated via git-stash isolation — same 2 WatcherPool.updateSecret errors (credential-refresh-job.test.ts, statusline-usage-file.test.ts) plans 032/033/039/040 already document, neither touched by this diff's 5-file scope) spec-impact: none |
| 042 | Add a wall-clock quiet-hours gate for non-critical TTS on the presence-unknown / legacy fallback path | P2 | M | 041 | DONE (converted into openspec proposal `add-notification-quiet-hours-gate` and applied via `/apply:all` apply-2026-07-13-003 wave 1; main @ d41a9943, migration `0055_supreme_strong_guy.sql` applied live against homelab Postgres. `quiet_hours_*` columns + `isWithinQuietHours()` helper + `QuietHoursWiring`/`applyQuietHoursIfNeeded()` on both `send()`/`flush()` delivery points + `readQuietHoursSettings()` wired as the 6th `NotificationManager` arg; quiet-hours.test.ts 7/0, manager-quiet-hours.test.ts 5/0, notification-settings.test.ts 37/0 (3 new); `pnpm --filter @nexus/agent typecheck` clean aside from a pre-existing unrelated bead-rollup.test.ts error (nx-urcfu, filed separately)) spec-impact: none |
