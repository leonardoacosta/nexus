...
mod tests {
    ...
    fn parse_session_start() {
        ...
        match ev {
            SocketEvent::SessionStart { session_id, project, model, pid, .. } => {
    ...
    fn parse_notification() {
        ...
        let ev: SocketEvent = serde_json::from_str(json).unwrap();
        match ev {
            SocketEvent::Notification { message, message_type, channels } => {
    ...
    fn parse_agent_complete() {
        ...
        let ev: SocketEvent = serde_json::from_str(json).unwrap();
        assert!(matches!(ev, SocketEvent::AgentComplete { duration_ms: Some(5000), .. }));
    }
