...
fn escape_for_tmux(text: &str) -> String {
    // Replace all single quotes with: end-quote, literal quote, start-quote
...
mod tests {
    ...
    fn escape_with_single_quote() {
        // Input: it's fine
        // Expected: 'it'"'"'s fine'
        assert_eq!(escape_for_tmux("it's fine"), "'it'\"'\"'s fine'");
    }
    ...
    fn escape_key_name_is_quoted() {
        // "Enter" without quoting would be sent as the Enter key by tmux;
