...
- [ ] [1.1] [P-1] Fix `agent-client.ts`: add `x-nexus-secret: process.env.NEXUS_ATTACH_SECRET` header to `fetchWithRetry` and `startSession` fetch; fix `startSession` error path to use `res.text()` fallback instead of `res.json()` on non-ok response [owner:api-engineer] [beads:nx-54ae]
- [ ] [1.2] [P-1] Fix `agent-routing.ts` `resolveAttachAgent`: return `{ agentName: string; isFallback: boolean }` using `loc.agentName` not `loc.agentId`; update `ProjectRow` and `ProjectCard` call sites accordingly [owner:api-engineer] [beads:nx-r9ew]
