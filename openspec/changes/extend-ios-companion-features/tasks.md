...
- [ ] [2.3] Extend `deploy/hooks.d/*/04-swift-deploy` + `deploy/lib/macos-swift-deploy.sh` to also build nexus-ios and run `xcrun devicectl device install app` against the paired iPhone via the existing GUI-agent LaunchAgent kickstart (nx-detes) [owner:devops-engineer] [type:ci-cd] [beads:nx-txuau]
