...
- [ ] 1.1 Reproduce the red baseline: `bash scripts/lint-sql-safety.sh` shows exactly one [beads:nx-k6c27]
      violation at `apps/web/src/lib/elevenlabs-client.ts:155`. (plans/023 Step 1)
- [ ] 1.2 Add `// SAFE: HTTP route in an error message, not SQL` trailing comment at [beads:nx-zpni6]
      `apps/web/src/lib/elevenlabs-client.ts:155`. (plans/023 Step 2)
- [ ] 1.3 Harden `scripts/lint-sql-safety.sh` Pattern 1 regex (line 49) to [beads:nx-yp647]
...
- [ ] 1.4 Prove detection power is preserved via the ephemeral `/tmp` fixture (7 true-positive SQL [beads:nx-zqpr7]
      shapes retained, 2 HTTP-verb shapes excluded) and the in-repo SAFE-annotation count (8).
      (plans/023 Step 4)
- [ ] 1.5 Run `pnpm typecheck`, `pnpm lint`, `bun test apps/web/src/lib` — no failure referencing [beads:nx-8x02z]
      a changed file. (plans/023 Step 5)
- [ ] 2.1 Rewrite `SchemaIncompleteError`'s remediation sentence in [beads:nx-b29kp]
      ...
      (plans/024 Step 1)
- [ ] 2.2 Update `database.test.ts`'s message assertion and add the PG-free [beads:nx-oc3yr]
...
- [ ] 2.3 Fix the three sibling prose sites: `docker-compose.test.yml:11`, [beads:nx-8olet]
...
- [ ] 2.4 Create `scripts/hooks/pre-commit-block-db-push.sh` (copy from [beads:nx-yp1fu]
...
- [ ] 2.5 Wire the guard into `.beads/hooks/pre-commit` AFTER the beads `# --- END BEADS [beads:nx-yltsr]
...
- [ ] 2.6 Full gates: `pnpm typecheck`, `pnpm lint`, `bun test apps/agent/src/db/database.test.ts`, [beads:nx-scyzd]
