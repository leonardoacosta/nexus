...
- [x] [2.5] [P-2] Wrap fetches in `apps/nextjs/src/app/actions/notifications.ts` with `withFailover` so the `/notifications/settings` GET and PATCH transparently fail over [owner:api-engineer] [type:api] [beads:nx-06bqd]
- [x] [2.6] [P-2] Wrap fetches in `apps/nextjs/src/app/actions/elevenlabs-credentials.ts` and `apps/nextjs/src/app/api/notifications/stream/route.ts` with `withFailover` for the credential read path. SSE proxy path uses `probeAgents()` to pick the active agent at connection time; failover within an open SSE stream is OUT of scope [owner:api-engineer] [type:api] [beads:nx-853jn]
- [x] [2.7] [P-2] Unit tests in `apps/nextjs/src/lib/__tests__/agent-reachability.test.ts`: 1st-up returns ok no-probe-2nd, 1st-down 2nd-up returns ok+failover, all-down returns last-attempt failure with attempts[], stale-binary on 1st falls through to 2nd [owner:test-writer] [type:testing] [beads:nx-s35k8]
- [x] [2.8] [P-2] Unit tests in `apps/nextjs/src/lib/__tests__/agent-failover.test.ts`: cache-hit skips network, cache-miss reprobes after TTL, withFailover retries on network error and updates cache, withFailover does NOT retry on 4xx, all-fail throws aggregated error and clears cache [owner:test-writer] [type:testing] [beads:nx-iuse8]
