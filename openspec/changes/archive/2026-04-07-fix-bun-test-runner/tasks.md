...
- [x] 1.1 In `apps/nextjs/src/__tests__/acceptance/ac-13-projects-page.test.tsx` (line 85),
...
- [x] 1.2 Repeat for the "shows empty state when no projects" test in the same file if it also
  uses a now-stale text assertion.

## 2. Fix AC-15 empty-state assertion

- [x] 2.1 In `apps/nextjs/src/components/__tests__/ac-15-projects-discovered.test.tsx` (line 72),
...
- [x] 3.1 In the same file (line 79), replace `screen.getAllByText("0 active")` with a query
...
- [x] 4.1 Run `cd apps/nextjs && bun run test` (which invokes `vitest run`) and confirm:
  - 0 test files failing
  - All 204+ tests pass (currently 200 pass / 4 fail)
- [x] 4.2 Confirm no regressions in unrelated test files.
