...
- [x] [1.1] [P-1] Create TF Cloud workspace `nx-prod` at app.terraform.io (CLI-driven, Local execution mode) [owner:user] → REQUIRES USER ACTION: Create TF Cloud workspace manually at app.terraform.io (org: priceless-dev, workspace: nx-prod) [beads:nx-67dz]
...
- [x] [4.1] [P-1] Run `pnpm tf init` — verify TF Cloud workspace connects and providers download [owner:user] → REQUIRES USER ACTION: Fill in infra/.secrets.env tokens then run `pnpm tf init` [beads:nx-t21b]
- [x] [4.2] [P-1] Import existing Cloudflare DNS record: `terraform import module.cloudflare.cloudflare_dns_record.nexus_homelab <zone_id>/<record_id>` [owner:user] → REQUIRES USER ACTION: Run `pnpm tf init` first, then `terraform -chdir=infra/environments/prod import module.cloudflare.cloudflare_dns_record.nexus_homelab <zone_id>/<record_id>` [beads:nx-ui58]
- [x] [4.3] [P-2] Run `pnpm tf plan` — verify no unintended changes shown [owner:user] [beads:nx-3u5y]
- [x] [4.4] [P-2] Run `pnpm tf apply` — verify `.tf-outputs.env` written and contains all four outputs [owner:user] [beads:nx-8p42]
- [x] [4.5] [P-3] Push to main and verify hook sources `.tf-outputs.env` and restarts agent with injected env vars [owner:user] [beads:nx-xyns]
