...
`claude_code.*` telemetry from the homelab InfluxDB (see capability `cc-telemetry-read`) rather than
self-exporting spans via an OTEL SDK. There is no Rust OTel layer to compose and no nexus-side OTLP
exporter.
**Migration**: Per-session cost / token observability is provided by the `cc-telemetry-read`
capability (reading `claude_code.cost.usage` / `claude_code.token.usage` from InfluxDB). Self-export
of nexus application spans is intentionally not pursued; if app-level tracing is wanted later it
will be specified as a new requirement against the Bun/TS runtime, not reinstated here.
