...
- [ ] [2.1] Add read-only VictoriaMetrics client `apps/agent/src/telemetry/vm-read.ts` (PromQL/HTTP against `VM_URL`, default `http://172.20.0.200:8428`, degraded no-op when unset, query-only — no remote-write) [owner:api-engineer] [beads:nx-4xmyw]
- [ ] [2.2] Add cost/token read service sourcing per-session cost from `claude_code_cost_usage_USD_total` + per-type tokens from `claude_code_token_usage_total`, keyed by `session_id` + `project` label. MUST filter `session_id=~".+"` (excludes the label-collision series cc's own dashboard had to filter — see proposal.md's Why section) [owner:api-engineer] [beads:nx-a3jx4]
- [ ] [2.3] Repoint `GET /sessions/{id}/tokens` to the VictoriaMetrics read service; empty breakdown + HTTP 200 when no series [owner:api-engineer] [beads:nx-t1jwv]
...
- [ ] [2.6] Add `VM_URL` to `deploy/secrets.env.example` + agent startup config (default `http://172.20.0.200:8428`; no auth token needed — VM has no auth layer on this internal-only endpoint) [owner:api-engineer] [beads:nx-8fbrq]
