...
- [ ] [2.1] [P-1] Add `packages/core/src/types/session-context.ts` — shared Zod schemas for the PATCH body and GET response (see proposal.md § Requirements for exact field shapes). Export from `packages/core/src/index.ts`, mirroring `types/integrations.ts`'s conventions. [owner:types-engineer] [type:api] [beads:nx-sh5rm]
