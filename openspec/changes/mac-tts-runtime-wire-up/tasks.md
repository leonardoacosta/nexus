...
- [ ] [1.1] Add `SystemSpeechSynthesizer` wrapper in `apps/swift/NexusShared/Synthesis/SystemSpeechSynthesizer.swift` — thin AVSpeechSynthesizer adapter with `speak(_ text: String, rate:)` method [owner:ui-engineer] [type:feature] [beads:nx-wy3i8]
- [ ] [1.2] Add `TTSObserver` class in `apps/swift/NexusShared/Observers/TTSObserver.swift` with `start() async` + `stop()` API, dependency-injected ElevenLabsClient + SystemSpeechSynthesizer + AudioPlayer + KeychainStore [owner:ui-engineer] [type:feature] [beads:nx-9dkr8]
- [ ] [1.3] [P-1] TTSObserver receives NotificationFired via `consumeNotifications`, filters channel=="tts", posts UNUserNotificationCenter banner BEFORE synth attempt [owner:ui-engineer] [type:feature] [beads:nx-hdriz]
- [ ] [1.4] [P-1] TTSObserver synth chain: try ElevenLabs (Keychain key, HTTP) -> fallback to SystemSpeechSynthesizer on any error; pipe ElevenLabs mp3 to AudioPlayer.shared.play with ducking from settings [owner:ui-engineer] [type:feature] [beads:nx-n7a8d]
