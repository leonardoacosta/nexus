...
- [x] [3.2] Push + ssh-pull homelab: hook chain runs bun install (if needed), 02-deploy rebuilds, agent restarts. Verify via `curl /sessions` that gitOwnerRepo / gitProvider are populated on active rows (your 3 active sessions in `/home/nyaptor/dev/oo` should show `leonardoacosta/oo`). **VERIFIED** after nx-lebux fix shipped: process-watcher now refreshes `cwd` from `/proc/<pid>/cwd` when the stored value is empty (pre-resolver legacy rows), then re-enriches via the existing resolver loop. See nx-lebux close note for curl evidence (active rows now have gitOwnerRepo populated) [owner:devops-engineer] [type:test] [beads:nx-cc4kj]
