...
- [ ] [3.2] Push + ssh-pull homelab: hook chain runs bun install (if needed), 02-deploy rebuilds, agent restarts. Verify via `curl /sessions` that gitOwnerRepo / gitProvider are populated on active rows (your 3 active sessions in `/home/nyaptor/dev/oo` should show `leonardoacosta/oo`). **BLOCKED on nx-lebux**: push 5d1ba34, ssh pull, post-merge hooks ran (no dependency changes; agent rebuilt in 0s; systemd reload reported active running PID 3047044 at 21:28:01 CDT), but /sessions returns 3 active rows with `cwd: ""` and `gitProvider: null, gitOwnerRepo: null`. Root cause: process-watcher line 196 enrichment guard `if (!row.gitProvider && row.cwd)` skips rows whose original cwd was empty. Fix scoped out to nx-lebux (P1) [owner:devops-engineer] [type:test] [beads:nx-cc4kj]
