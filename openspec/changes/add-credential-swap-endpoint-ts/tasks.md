...
- [x] [1.1] [P-1] Add `ManualSwapResult` type and `manualSwap(targetId: string)` method to `apps/agent/src/credentials/pool.ts` — parks current best-available (status=cooldown, cooldownUntil=now+cooldownMs, rateLimitCount unchanged), emits manual_swap credential_events rows for parked + activated [owner:api-engineer] [beads:nx-rzcm]
- [x] [1.2] [P-2] Add `handleSwapCredential` handler in `apps/agent/src/routes/credentials.ts` — parses `{ to: string }` body, name→primaryId lookup, calls pool.manualSwap(), emits two audit entries (credential.manual_swap_out + credential.manual_swap_in), returns pool list [owner:api-engineer] [beads:nx-za4q]
- [x] [1.3] [P-2] Register `POST /credentials/swap` route in `apps/agent/src/routes.ts` after the existing credential routes [owner:api-engineer] [beads:nx-d8wd]
