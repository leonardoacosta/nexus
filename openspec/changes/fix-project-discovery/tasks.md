...
- [ ] 1.1 Replace `hasActiveSessions: boolean` with `activeSessions: number` and
      `totalSessions: number` in `AgentDiscoveredProject` interface
      (`apps/agent/src/routes/projects-discovered.ts:27-31`)
- [ ] 1.2 Update route handler to count active sessions (sessions with `status = "active"` or
      `last_seen` within 5 min) and total sessions from `recentSessions` result set
- [ ] 1.3 Populate `activeSessions` and `totalSessions` on each `AgentDiscoveredProject` entry
      before pushing to the `projects` array
- [ ] 1.4 Update `AgentDiscoveredProjectsResponse` if field names or types changed; ensure
      TypeScript compiles cleanly (`bun run typecheck`)

## 2. Tilde Expansion — Guard All Call Sites (P1)

- [ ] 2.1 Audit every location that reads `agent.projectsDir` from the DB; confirm
      `expandProjectsDir` is called before any FS operation
- [ ] 2.2 Add a unit test for `expandProjectsDir` covering: plain `~/dev`, `~` only,
      `/absolute/path` unchanged, relative path without `~` unchanged
- [ ] 2.3 Add integration test: agent row with `projects_dir = "~/dev"` must scan successfully
      (no ENOENT) when `os.homedir()` is mocked

## 3. Next.js Client — Count Mapping Fix (P1)

- [ ] 3.1 Remove `project.hasActiveSessions ? 1 : 0` expression from `fetchDiscoveredProjects`
      (`apps/nextjs/src/lib/agent-client.ts:190`)
- [ ] 3.2 Map `activeSessions` → `active_sessions` and `totalSessions` → `total_sessions` from
      the wire type to `DiscoveredProject`
- [ ] 3.3 When dedup merges a duplicate, accumulate `active_sessions` and `total_sessions`
      across agents rather than ignoring the second entry's counts
- [ ] 3.4 Update local type alias `AgentDiscoveredResponse` (or equivalent) to match the new
      wire format

## 4. Dedup Key — Normalized Path (P2)

- [ ] 4.1 Replace dedup key `${project.name}|${project.path}` with normalized absolute path:
...
- [ ] 4.2 Attempt to read git remote URL from `git remote get-url origin` for each project
      ...
      to normalized path
- [ ] 4.3 On dedup hit, accumulate counts (step 3.3) and do NOT overwrite the `agent` field
...
- [ ] 5.1 Add `lastSeenAt: number` (Unix ms timestamp) to the aggregated discovered project
      entry (client-side, not wire format)
- [ ] 5.2 On each `fetchDiscoveredProjects` call, remove entries whose `lastSeenAt` is older
...
- [ ] 6.1 In route handler, resolve symlinks via `fs.realpathSync` before inserting into
      `projects` array; if resolution fails (broken symlink) skip the entry and log a warning
- [ ] 6.2 Skip entries that already exist at the resolved path (guards against `link → real`
...
- [ ] 8.1 Confirm `agent` field is set only on first-seen entry (step 4.3 covers this)
- [ ] 8.2 When a project appears on multiple agents, surface `machineCount` to the UI layer
      (already tracked — ensure it is not dropped before the component receives it)

## 9. Cache Freshness Header (P3)

- [ ] 9.1 Add `X-Cache-Age: <ms>` response header to `GET /projects/discovered` indicating age
      of cached data (0 when freshly computed)
- [ ] 9.2 Add `X-Cache-TTL: <ms>` response header with configured TTL value

## 10. Empty projectsDir Feedback (P3)

- [ ] 10.1 When `rawProjectsDir` is empty after trim, log at `info` level with a clear message:
      `"projectsDir not configured for agent <id> — returning empty project list"`
- [ ] 10.2 Include `configured: false` field in the response body when `projectsDir` is
      unconfigured (additive, non-breaking)

## 11. Input Validation (P3)

- [ ] 11.1 Reject `projectsDir` values containing `..` path segments after expansion (return
      `400` with descriptive error)
- [ ] 11.2 Reject empty string after trim (handled by step 10.2, return empty list — do not
      error)
- [ ] 11.3 Add validation unit tests for traversal attempt and valid deep path
...
- [ ] 13.1 `bun run typecheck` passes with zero errors across all packages
- [ ] 13.2 `bun run lint` passes with zero warnings on changed files
- [ ] 13.3 All new unit tests pass: `bun test --filter projects-discovered`
