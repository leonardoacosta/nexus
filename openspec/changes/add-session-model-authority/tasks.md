...
- [ ] 2.1 Add `packages/core/src/model-letter.ts` exporting `modelFamilyLetter(model?: { id?: [beads:nx-w581i]
...
- [ ] 2.2 Add a write-through helper in `apps/agent/src/db/sessions.ts`, [beads:nx-zkqs2]
...
- [ ] 2.3 In `apps/agent/src/services/process-hook-event.ts`, call `updateSessionModel` when [beads:nx-cnql9]
...
- [ ] 2.4 In `apps/agent/src/services/socket-server/dispatcher.ts`'s `session_heartbeat` case, [beads:nx-6jxsv]
      ...
      - depends on: (none — task 2.2 lands first within this same batch)
- [ ] 2.5 In `apps/agent/src/routes/statusline.ts`, replace the hardcoded `model: null` (line [beads:nx-xgt36]
...
- [ ] 2.6 Update `apps/nexus-statusline/src/render.ts` to import `modelFamilyLetter` from [beads:nx-pun8v]
      ...
      - depends on: (none — task 2.1 lands first within this same batch)
- [ ] 2.7 Unit tests: `packages/core`'s `model-letter.test.ts` (ported from [beads:nx-5qs3u]
