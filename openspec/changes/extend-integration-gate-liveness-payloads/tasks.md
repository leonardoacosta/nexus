...
- [ ] [1.2] [P-1] Add `lastTickMs(): number` getter to `processWatcher` in `apps/agent/src/services/process-watcher.ts`, updated at the end of `reconcileOnce` [owner:api-engineer] [type:feature] [beads:nx-e603d]
- [ ] [1.3] [P-1] Add `isListening(): boolean` getter to `socketServer` in `apps/agent/src/services/socket-server.ts` [owner:api-engineer] [type:feature] [beads:nx-fyp31]
