...
- [x] [3.1] Verify `deploy/hooks.d/pre-push/01-deploy` Tier A bun-test invocation already exports `NEXUS_HEAVY_TESTS=1` for the new tests (no change expected; this task confirms) [owner:devops-engineer] [type:test] [beads:nx-k31ct] — flag was MISSING (homelab-transport.test.ts:48 `heavyEnabled` gate would skip the whole file). Added `NEXUS_HEAVY_TESTS=1` env prefix to the `pnpm test` invocation at line ~85 of `01-deploy`.
- [x] [3.2] Verify `deploy/hooks.d/pre-push/01-deploy` Tier A xcodebuild invocation already runs the `NexusSharedTests` bundle (it does — task confirms the new test class is picked up automatically once the file lands) [owner:devops-engineer] [type:test] [beads:nx-0jrpa] — bundle was NOT picked up: `NexusSharedTests` depends on `NexusShared` (framework), not `nexus-mac` (app), so `-only-testing:nexus-mac-Tests` excluded it. Added second `-only-testing:NexusSharedTests` flag alongside the existing one. Verified via `xcodebuild -scheme nexus-mac -showBuildSettings -only-testing:NexusSharedTests` (scheme resolves cleanly).
