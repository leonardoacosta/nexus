...
/// Legacy menu-bar type name. Now an alias for the cross-platform
/// `NexusShared.HealthSnapshot`. Shape is identical (JSON keys match
/// `apps/agent/src/routes/health-history.ts:9`) — renamed in NexusShared to
/// emphasise "point-in-time snapshot" semantics. Menu-bar call sites keep
/// using `HealthPoint` to avoid churn.
typealias HealthPoint = NexusShared.HealthSnapshot
