 ...
 *
 * Auth gate dropped in `drop-attach-secret-gate` — all REST + WebSocket
 * routes now respond without an `x-nexus-secret` header. Reachability is
 * enforced at the bind layer (loopback + Tailscale only).
 */
...
// ---------------------------------------------------------------------------
// WebSocket endpoints — no auth gate (drop-attach-secret-gate)
// ---------------------------------------------------------------------------

describe("Agent API: WebSocket endpoints", () => {
  it("GET /sessions/{id}/stream returns 404 when no PTY is attached", async () => {
    const res = await fetch(`${baseUrl}/sessions/no-such-session/stream`);
    expect(res.status).toBe(404);
  });

  it("GET /sessions/{id}/interact returns 404 when no PTY is attached", async () => {
