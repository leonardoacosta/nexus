 ...
 *
 * Mirrors the shape of `server-routes-credentials.ts` — five DB-backed routes
 * dispatched by `server-request-handler.ts`. There is no per-request auth
 * middleware (the legacy `x-nexus-secret` gate was removed by
 * `drop-attach-secret-gate`; reach is bounded at the bind layer — loopback +
 * Tailscale only).
 *
 * Routes:
 ...
 *
 * All five require a Db handle; when `db` is undefined the dispatcher returns
 * null so the caller falls through to the not-found branch.
 *
...
export function tryHandleElevenlabsRoute(
  ...
  if (url.pathname === "/elevenlabs/credentials" && request.method === "GET") {
  ...
  if (url.pathname === "/elevenlabs/credentials" && request.method === "PATCH") {
