 ...
 *
 * Endpoints (no per-request auth gate; reach is bounded at the bind layer —
 * loopback + Tailscale only — after `drop-attach-secret-gate`):
 ...
 *
 * The encryption key is resolved fresh from `NEXUS_ENCRYPTION_KEY` per mutation
 * via `tryLoadEncryptionKey()`; a missing/malformed key yields HTTP 400 rather
 * than a crash. The db handle is passed in by the dispatcher.
 *
